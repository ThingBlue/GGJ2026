...
public class DialogueOptionButton : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public float foregroundAlphaSmoothTime;
    ...
    private void Update()
    {
        ...
        float borderAlpha = Mathf.MoveTowards(border.color.a, targetForegroundAlpha, (1.0f / foregroundAlphaSmoothTime) * Time.deltaTime);
        float textAlpha = Mathf.MoveTowards(text.color.a, targetForegroundAlpha, (1.0f / foregroundAlphaSmoothTime) * Time.deltaTime);
        ...
        if (targetForegroundAlpha == 0)
        {
            ...
            float hoveredEffectAlpha = Mathf.MoveTowards(arrow.color.a, targetForegroundAlpha, (1.0f / foregroundAlphaSmoothTime) * Time.deltaTime);
        ...
        if (borderAlpha <= 0 && textAlpha <= 0) Destroy(gameObject);
    }

    public void OnPointerEnter(PointerEventData eventData)
    {
        if (targetForegroundAlpha == 0) return; // Don't change current hover target when an option has already been selected
        hovered = true;
